...
function 색칠유형대로색칠() {
  console.log('색칠유형대로색칠() 색칠유형 : ' + 색칠유형)
  var 색칠할곳=document.querySelectorAll('.색칠묶음div p');
  if (색칠유형=='') {
    console.log('색칠유형대로색칠()')
    console.log('색칠유형 : 없음.종료 ')
    return;
  }

  document.querySelector('#색칠유형확인').innerHTML='색칠유형==' + 색칠유형;
  console.log('색칠유형대로색칠()')
    ...
    if (document.querySelector('#' + 색칠유형)) {
      ...
      for (var i=0; i<있다면본인색칠할요소를포함하는p요소들.length; i++) {
        if (있다면본인색칠할요소를포함하는p요소들[i].classList.contains('색칠제외') || 있다면본인색칠할요소를포함하는p요소들[i].innerHTML=='') {
          ...
          색칠할번호들일반.push(있다면본인색칠할요소를포함하는p요소들[i].innerHTML);
  ...
  if (색칠유형=='색칠유형_노란색번호색칠') {
    ...
    for (var i=0; i<있다면본인색칠할요소를포함하는p요소들.length; i++) {
      if (있다면본인색칠할요소를포함하는p요소들[i].classList.contains('피할번호연계')) {
        색칠할번호들노랑.push(있다면본인색칠할요소를포함하는p요소들[i].innerHTML);
      }
    }
  }
  if (색칠유형=='색칠유형_주황색번호색칠') {//일반색칠 그대로 두고... 중복시 중복색칠
    색칠할번호들주황=[];
    console.log('색칠유형 : ' + 색칠유형)  
    있다면본인색칠할요소를포함하는p요소들=document.querySelectorAll('#js피해서색칠 button');
    //ex:배치_오른쪽_회차선택과_조회_선택회차당번(p10개), 미출수5주간, 미출수10주간, 미출수15주간, 오주1~5출수
  ...
  console.log('색칠할번호들일반 : ' + 색칠할번호들일반);
  console.log('색칠할번호들노랑 : ' + 색칠할번호들노랑);
  console.log('색칠할번호들주황 : ' + 색칠할번호들주황);

  //색칠하기
  for (var i=0; i<색칠할곳.length; i++) {
    if (색칠할번호들일반[0]) {
      if (색칠할곳[i].innerHTML!='' && 색칠할번호들일반.find(element => element==색칠할곳[i].innerHTML)) {색칠할곳[i].classList.add('일반또는노랑')}
    }
    if (색칠할번호들노랑[0]) {
      if (색칠할곳[i].innerHTML!='' && 색칠할번호들노랑.find(element => element==색칠할곳[i].innerHTML)) {색칠할곳[i].classList.add('일반또는노랑')}
    }  
    if (색칠할번호들주황[0]) {
      if (색칠할곳[i].innerHTML!='' && 색칠할번호들주황.find(element => element==색칠할곳[i].innerHTML)) {
        if (색칠할곳[i].classList.contains('일반또는노랑')) {
          색칠할곳[i].classList.remove('일반또는노랑')
          색칠할곳[i].classList.add('겹침표시');
        } else {
          색칠할곳[i].classList.add('주황단독')
        }
      }
    }         
  }
  ...
  //추가, 주황누르면 주황으로 누적되도록
  if (색칠유형=='색칠유형_노란색번호색칠' || 색칠유형=='색칠유형_주황색번호색칠') {
    if (색칠할번호들노랑및일반[0]) {
...
function 클릭이벤트통합(e) {
  ...
  if (아이디=='색칠유형_최근회차'||아이디=='색칠유형_미출수5주'||아이디=='색칠유형_미출수10주'||아이디=='색칠유형_미출수15주'||
      ...
      아이디=='색칠유형_오주4출수'||아이디=='색칠유형_오주5출수'||아이디=='색칠유형_다음') {
        console.log('클릭이벤트통합(e), 색칠유형 : ' + 아이디);
        색칠유형=아이디;색칠해제();색칠유형대로색칠();
  }
  if (아이디=='색칠유형_노란색번호색칠') {console.log('클릭이벤트통합(e), 색칠유형 : ' + 아이디);색칠유형=아이디;색칠해제();색칠유형대로색칠();}
  if (아이디=='색칠유형_주황색번호색칠') {console.log('클릭이벤트통합(e), 색칠유형 : ' + 아이디);색칠유형=아이디;색칠해제();색칠유형대로색칠();}  

  if (아이디=='페이지1') {변경할페이지=아이디; 페이지변경();return;}
  if (아이디=='js색칠해제와변수초기화' || 아이디=='색칠유형_노랑주황색칠해제') {색칠유형및번호초기화();색칠해제();return;}
...
function 색칠해제() {//class 제거 : 본인색칠, 왼쪽색칠
  document.querySelector('#색칠유형확인').innerHTML='';
  var 색칠할곳=document.querySelectorAll('.색칠묶음div p');
  for (i=0; i<색칠할곳.length; i++) {
    색칠할곳[i].classList.remove('일반또는노랑');
    색칠할곳[i].classList.remove('주황단독');
    색칠할곳[i].classList.remove('겹침표시');
  }
}
function 회차변경실행() {
  console.log('회차변경실행()');
  ...
  if ('회차selectedIndex에따라_최근번호날짜와번호넣기'=='회차selectedIndex에따라_최근번호날짜와번호넣기') {
    ...
    for (var i=3; i<9; i++) {//P가 10개다. 다른것과 맞추기위해
      document.querySelectorAll('#span_날짜_색칠묶음div p')[i-3].innerHTML=당번전체[-(회차selectedIndex*9) + 당번전체.length - 7 + (i-3)]; 
    }
    document.querySelectorAll('#span_날짜 ~ p')[3].innerHTML=당번전체[-(회차selectedIndex*9) + 당번전체.length - 7 + (9-3)]; 
  }
  ...
  if ('아래 회귀변경 숫자 생성'=='아래 회귀변경 숫자 생성') {
    ...
    for (var 값이있으면=0; 값이있으면<회귀결과정보수; 값이있으면++) {
      if (당번전체[-((회차selectedIndex+(회귀숫자*(값이있으면+1)))*9) + 당번전체.length - 8]) {
        js_회귀변경p생성html+='<P></P><P></P><P></P><div class="색칠묶음div"><P></P><P></P><P></P><P></P><P></P><P></P></div><P></P><br>'
      }
    }
    ...
    document.querySelector('#js_회귀변경p생성').innerHTML='<P></P><P></P><P></P><div class="색칠묶음div"><P></P><P></P><P></P><P></P><P></P><P></P></div><P></P><br>' + document.querySelector('#js_회귀변경p생성').innerHTML;
  ...
  if ('미출수기록'=='미출수기록') {
    ...
    // 몇개 부분 없애기 전
    미출수5_html='<div id="미출수5주간"><span>5주간미출</span><span id="색칠유형_미출수5주">색칠</span><div class="색칠묶음div">' + 미출수5_html + '</div></div>';
    미출수10_html='<div id="미출수10주간"><span>10주간미출</span><span id="색칠유형_미출수10주">색칠</span><div class="색칠묶음div">' + 미출수10_html + '</div></div>';
    미출수15_html='<div id="미출수15주간"><span>15주간미출</span><span id="색칠유형_미출수15주">색칠</span><div class="색칠묶음div">' + 미출수15_html + '</div></div>';
    ...
    for (var i=minValue;i<=maxValue; i++) {//최대 최소 사이의 값이 없을수도 있음.
      ...
      if (i==0) {//오주0출수는 만들지 않는다
        ...
        if (횟수담기.filter(element => i == element).length>0) {
          횟수만html+='<div id="오주' + i + '출수"><span>오주' + i + '출수</span><span id="색칠유형_오주' + i + '출수">색칠</span><div class="색칠묶음div">'
          //개수확인
          개수확인=0;
          for (var 번호순회=0;번호순회<45;번호순회++) {
