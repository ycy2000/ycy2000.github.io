...
function 연습() {
  ...
  //정답 4523
  let 기회들 = [
    {기회숫자:[0,1,2,3], 숫자일치:0, 위치일치:2},
    {기회숫자:[6,7,8,9], 숫자일치:0, 위치일치:0},
    {기회숫자:[8,9,2,3], 숫자일치:0, 위치일치:2},
    {기회숫자:[5,4,2,3], 숫자일치:2, 위치일치:2},
  ]
  let 모든조합=[[0,1,2,3],[9,1,2,4],[8,1,2,5],[4,5,3,2],[4,5,2,3]]
  //2.모든조합을 비교 : 기회숫자는 위치일치 확인,
  모든조합.forEach((조합,index) => {
    console.log(index, '번째 기회숫자')
    let 유효 = true;
    for (const { 기회숫자, 숫자일치, 위치일치 } of 기회들) {
      ...
      console.log('          ', 기회숫자)
      console.log('          ', 조합, ', 위치일치 : ', 위치일치, ',', 위치일치수,  ', 숫자일치 : ', 숫자일치, ', ',숫자일치수)
