if ('초기설정' == '초기설정') {
  //양이 많아서 숨길것을 위로 두고 기록중인것을 아래로 두는데 합쳐야되기 때문에..... 합친후 '배열당번정보'에 모든배열을 담는다.
  var 배열당번정보 = [];
  $('.당번한줄').each(function () {
      배열당번정보.push(
          $(this)
...
function 연습() {
  let 문자열 = document.querySelector('#숨김정보_당번전체').innerText;
  ...
  for (let i = 0; i < arr.length; i += 9) {
    ...
    let 회차 = arr[i];
    let 날짜 = arr[i + 1];
    let 당번6 = arr.slice(i + 2, i + 8).join('_');
    let 보너스 = arr[i + 8];
    결과문자열 += `\n<div class="당번한줄"><div>${회차}</div><div>${날짜}</div><div class="당번6">${당번6}</div><div>${보너스}</div></div>`;
  }
...
function 고정html_구조생성및_초기설정() {
  ...
  //회차select옵션생성
  let 옵션 = 배열당번정보.map(v => `<option>${v[0]}</option>`).reverse().join('');
  ...
  if ('분석자료_표_하_js' == '분석자료_표_하_js') {
    ...
    if ('오른쪽' == '오른쪽') {
      ...
      for (var 외부 = 0; 외부 < 50; 외부++) {
        ...
        for (var i = 1; i < 46; i++) {
          var div요소 = document.createElement('button'); // div 요소 변수에 담는다. 다섯개씩번갈아색칠
          if (외부 == 0 || 외부 == 19) { div요소.innerText = i }
          번호45.appendChild(div요소);
        }
    ...
    if ('왼쪽' == '왼쪽') {
      ...
      <div style="border:1px solid black;border-right:0px;">누적출-2</div>
      <div style="border:1px solid black;border-right:0px;">누적출-1(출현시 간격)</div>
      <div style="border:1px solid black;border-right:0px;">-1 : 단일 간격 출 횟수</div>
      <div style="border:1px solid black;border-right:0px;">-2 : 두개 간격 일치 경우...</div>
      <div style="border:1px solid black;border-right:0px;">이전 : 횟수1회 차감</div>
      <div style="border:1px solid black;border-right:0px;">(출현시 차감)</div>
...
function 임시보관용() {
    ...
    $('.당번6').each(function () {
        $(this).text().split('_').forEach(function (번호) {
            횟수[Number(번호)]++;
        });
    });

    $('.총출현횟수 button').each(function (i) {
        $(this).text(횟수[i + 1]);
    });
}
...
function 당번_회차change설정() {
  ...
  const getNumbersRow = (idx) => 배열당번정보[idx]; // 문자열 배열
  ...
  for (let 이십주 = 0; 이십주 < TWENTY_WEEKS; 이십주++) {
    ...
    console.log('최근회차 : ' + 최근회차)
  }

  // ===== 다음회차 버튼 영역 =====
  if (배열당번정보[Number(회차) + 1]) {
...
function 분석자료_회차change설정() {
  ...
  if (회차 > 최근회차) { 
    console.log('회차가 최근회차보다 크면 종료'); 
    회차 = 회차 - 1; 
    document.querySelector('#분석자료_회차select').value = Number(회차); return; 
  } //회차=회차-1 ==> 원래대로 돌림
  if (1 == 1) { //1. 당번8칸채우기(30회분), 다음회차 채우기(둘다), 설정유형별로 회차 적용하기
    if (배열당번정보[Number(회차) + 1]) {
      for (var i = 0; i < 9; i++) {
        document.querySelectorAll('#분석자료_다음회차 > button')[i].innerHTML = 배열당번정보[Number(회차) + 1][i];
      }
    ...
    for (var i = 1; i < 9; i++) {//날짜부터 번호 8종
      document.querySelectorAll('#분석자료_선택회차 > button')[i - 1].innerHTML = 배열당번정보[Number(회차)][i];
    }
  }
  if (1 == 1) { //#당번_오주삼십주개수 작성 및 변수에 값 넣기, //#당번변수 : 안에 class가 #분석자료변수 안에도 동일하므로... 부모id
    ...
    if (i == 0 && 회차 != 최근회차) { document.querySelector(`${부모id} .${변수명순서대로[0]}`).innerHTML = 배열당번정보[회차 + 1].slice(2, 8).join(',') } //0 공통변수_다음당번
    var 좌우수 = [], 이웃수 = [];
    당번 = 배열당번정보[회차].slice(2, 8);
    document.querySelector(`${부모id} .공통변수_당번`).innerHTML = 배열당번정보[회차].slice(2, 8).join(',');
    document.querySelector(`${부모id} .공통변수_당번이웃`).innerHTML = 배열당번정보[회차].slice(2, 8).join(',') + ',';

    배열당번정보[회차].slice(2, 8).forEach(숫자 => {
    ...
    좌우수.forEach(숫자 => {
      if (!배열당번정보[회차].slice(2, 8).includes(숫자)) {
        document.querySelector(`${부모id} .공통변수_이웃`).innerHTML += 숫자 + ',';
        document.querySelector(`${부모id} .공통변수_당번이웃`).innerHTML += 숫자 + ',';
      };
    })
    ...
    for (var i = 0; i < 30; i++) {
      if (i < 5) { 배열당번정보[회차 - i].slice(2, 8).forEach(번호 => { 오주당번모음.push(번호) }) }
      if (i < 10) { 배열당번정보[회차 - i].slice(2, 8).forEach(번호 => { 십주당번모음.push(번호) }) }
      if (i < 15) { 배열당번정보[회차 - i].slice(2, 8).forEach(번호 => { 십오주당번모음.push(번호) }) }
      if (i < 30) { 배열당번정보[회차 - i].slice(2, 8).forEach(번호 => { 삼십주당번모음.push(번호) }) }
    }
    /* 
    0 공통변수_다음당번  1 공통변수_당번  2 공통변수_이웃  3 공통변수_당번이웃  4 공통변수_5주출  5 공통변수_5주0출  6 공통변수_5주1출  
    7 공통변수_5주2출   8 공통변수_5주3출  9 공통변수_10주0출  10 공통변수_15주0출  11 공통변수_30주0출  12 공통변수_30주1출  
  ...
  if ('다음당번이 있을때' == '다음당번이 있을때') {
    ...
    if (회차<최근회차) {
      for (var i=0; i<6; i++) {
        색칠할요소.children[Number(배열당번정보[Number(회차) + 1][i+2])-1].classList.add('다음당번표시');
      }
    }
  }
  분석자료_삼십회표_js작성();
   ...
   console.log('기준index', 회차);
    ...
    for (let 번호 = 1; 번호 <= 45; 번호++) {
        ...
        for (let n = 1; n <= 5; n++) {
            ...
            let 찾기 = "_" + 이전.slice(0, n).join("_") + "_";
...
function 분석자료_삼십회표_js작성() {
  if ('출수최대값' == '출수최대값') {
    ...
    for (var i = 1; i < 21; i++) { //i가 1부터임
      var 삼십주당번모음 = [];
      for (var 삼십주 = 0; 삼십주 < 30; 삼십주++) { 배열당번정보[회차 - 삼십주 - i + 1].slice(2, 8).forEach(번호 => { 삼십주당번모음.push(번호) }) }
  ...
  var 삼십주당번모음 = [];
  for (var i = 0; i < 30; i++) { 배열당번정보[회차 - i].slice(2, 8).forEach(번호 => { 삼십주당번모음.push(번호) }) }
...
function 분석자료_삼십회빈도와개수_js작성() {
  ...
  for (var i = 1; i < 21; i++) { //i가 1부터임
    출횟수[i].children[0].innerHTML = 배열당번정보[회차 - i + 1][0];
    var 삼십주당번모음 = [];
    for (var 삼십주 = 0; 삼십주 < 30; 삼십주++) { 배열당번정보[회차 - 삼십주 - i + 1].slice(2, 8).forEach(번호 => { 삼십주당번모음.push(번호) }) }
    if (최근회차 < (회차 - i + 2)) { var 다음회차당번 = [] } else { var 다음회차당번 = 배열당번정보[회차 - i + 2].slice(2, 8); }
    for (var 출45 = 1; 출45 < 46; 출45++) {
      var 출수 = 삼십주당번모음.filter(번호 => 번호 == 출45).length;
