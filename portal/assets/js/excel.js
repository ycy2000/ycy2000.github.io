...
function 개인정리png(e) {
  ...
  if (typeof e === 'object') { // 'this를 넣었을때'
    들어갈곳id=e.title.split(':')[0];
    파일이름=e.title.split(':')[1];
    들어갈요소id=e.title.split(':')[2];
  }
  ...
  if (파일이름!='없음') {
    ...
    document.querySelector(`#${들어갈곳id}`).innerHTML='<img src="' + src값 + '" alt="이미지없음" style="vertical-align: top;margin-right:5px;"></img>';
    document.querySelector(`#${들어갈곳id}`).classList.remove('d-none');
  }
  if (들어갈요소id!='없음') {
    document.querySelector(`#${들어갈곳id}`).innerHTML+=document.querySelector(`#${들어갈요소id}`).outerHTML;
    document.querySelector(`#${들어갈곳id}`).classList.remove('d-none');
  }
}
...
var 리스너_파이썬전체 = document.querySelector('#파이썬전체');
...
function header_클릭시(e) {
  ...
  if (e.target.innerHTML == 'Java') {//캔버스 들어가려면 클릭이 된다.
    ...
    if (e.target.innerHTML == '파이썬') {//캔버스 들어가려면 클릭이 된다.
    선택한캔버스id='파이썬전체';
    console.log('  선택한캔버스id : ' + 선택한캔버스id);
    document.querySelector('#캔버스이름').innerHTML=선택한캔버스id;
    선택한캔버스_카테고리작성및_초기작업();
...
리스너_파이썬전체.addEventListener('click', 선택한캔버스클릭시);
리스너_파이썬전체.addEventListener('change', 선택한캔버스_검색input_change시);
